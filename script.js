...
forecastBtn.addEventListener('click', async () => {
  ...
  card.querySelector('.city-click')?.addEventListener('click', () => playWeatherSound(fetchCity));
card.querySelector('.weather-icon')?.addEventListener('click', () => playWeatherSound(fetchCity));
...
function playWeatherSound(cityName) {
  const card = document.getElementById(`card-${cityName.replace(/\s+/g, '_')}`);
  if (!card) return;

  const desc = card.querySelector('.description').textContent.toLowerCase();
  let soundFile = "ambient.mp3";
  if (desc.includes("rain") || desc.includes("drizzle")) soundFile = "rain.mp3";
  else if (desc.includes("wind")) soundFile = "wind.mp3";
  else if (desc.includes("snow")) soundFile = "snow.mp3";
  else if (desc.includes("clear")) soundFile = "sunny.mp3";
  else if (desc.includes("heat") || desc.includes("hot") || desc.includes("sun")) soundFile = "heat.mp3";
